...
| A9 | **The estimated half-life is an unbiased guide to convergence time.** | It is not: OLS on the AR(1) inherits the Dickey-Fuller small-sample bias, so κ is under-estimated and half-lives come out **too long** — measured −20.5% in κ at κ=0.01, −3.5% at κ=0.20 (VALIDATION.md §9). Consequence: a time stop set at *k × estimated half-life* is systematically too loose and lets losers run. Mitigants: set `k` conservatively, cap the accepted half-life in the funnel, and prefer the shorter of the OLS/MLE estimates. Quantified in `test_properties.py::test_ols_kappa_is_biased_downward_and_worse_for_slow_reversion`. |
| A10 | **A mean-reverting OU fit implies a tradeable spread.** | It does not, and this is the trap the whole cointegration step exists to catch: on a *pure random walk* OLS returns `b` slightly below 1, so `OUFit.mean_reverting` is **True** with a spurious half-life of 300–950 days. The flag only trips at `b ≥ 1`, so it gives no protection. Mitigants: the funnel gates on Engle-Granger with **N=2** MacKinnon values *before* any OU fit is trusted, and caps the accepted half-life. Demonstrated in `test_properties.py::test_an_ou_fit_alone_cannot_reject_a_random_walk`. |
